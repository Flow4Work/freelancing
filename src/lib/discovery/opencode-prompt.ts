...
4. Reels 탭을 실제로 열고 최근 업로드 기준 Reel을 확인한다.
   - 최대 6개만 수집한다.
   - 오래된 고정(pinned) Reel은 제외한다. 첫 화면 순서를 그대로 최신순이라고 가정하지 않는다.
   - 실제 Reel이 6개 미만이면 확인 가능한 최근 Reel 전부를 사용한다.
   - 각 Reel마다 실제 URL과 실제 화면에 표시된 조회수를 확인한다.
   - postedAt을 실제로 확인할 수 있으면 함께 넣고, 확인하지 못하면 null로 둔다.
5. Reel 조회수를 실제로 읽은 경우에만 views에 정수를 넣는다. 조회수를 못 읽은 Reel을 0으로 넣거나 다른 수치로 추정하지 않는다.
6. 확인 대상으로 삼은 최근 Reel 중 하나라도 조회수를 읽지 못했다면 그 Reel의 views를 null로 제출한다. 서버가 해당 후보를 insufficient로 남겨 재검증 대상으로 처리한다.
7. reels:[]는 실제로 Reels 탭과 개별 Reel 접근을 시도했지만 확인 가능한 Reel이 없거나 접근 자체가 불가능한 경우에만 허용한다. 이 경우 note에 'Reels 원본 확인 불가' 또는 실제 이유를 반드시 적는다. reels:[] 상태를 검증 완료라고 해석하지 않는다.
8. 고정 Reel 여부나 최근 업로드 순서를 실제 화면에서 구분할 수 없다면 추정하지 않는다. 최신 unpinned Reel 집합을 신뢰할 수 없으면 조회수 검증 실패로 처리하고 note에 이유를 적는다.
...
- Instagram에서 조회수를 확인하지 못한 계정은 낮은 성과로 추정하지 않는다. 0회로 만들지 않고 views:null 또는 reels:[]로 제출하여 insufficient → 재검증으로 남긴다.
...
- available 공개 후보는 Reels 탭을 실제로 열었는지 다시 확인한다.
- 수집한 Reel은 최대 6개이고 오래된 고정 Reel이 섞이지 않았는지 확인한다.
- 각 수집 Reel에 실제 URL이 있고, 조회수를 읽었다면 실제 정수 views가 있는지 확인한다.
