...
검증 기준:
- exists: 현재 프로필이 실제 존재하면 true
- isPrivate: 비공개면 true, 공개면 false, 판단 불가면 null
- isPersonalCreator: 개인 크리에이터면 true. 병원/의사/브랜드/매장/서비스/미디어/공식 운영계정이면 false
...
- 고정(pinned) 여부 때문에 화면 상단에 보이는 순서를 최신순이라고 가정하지 않는다. 가능한 경우 각 Reel의 실제 게시일을 확인한다.
...
마지막에는 설명문 대신 아래 JSON 한 개를 정확히 만든다.
{
  "jobId": "${jobId}",
  "category": "${category}",
  "results": [
...
Windows라면 JSON을 UTF-8 임시파일로 저장한 뒤 curl.exe --data-binary @파일 방식으로 제출한다. 응답에 \"ok\":true가 확인되어야 작업 완료다. 제출 실패 시 임의로 성공 처리하지 말고 실패 사유만 보고한다.`;
}
