...
export function getSearchStageViewState(candidate: DiscoveryCandidate): "verification_needed" | "recommended" | "unmapped" {
  ...
  if (candidate.candidateStatus === "search_qualified") {
    const recommendation = getRecommendationAssessment(candidate);
    if (recommendation.tier === "ineligible") return "unmapped";
    return recommendation.tier === "review" ? "verification_needed" : "recommended";
  }
...
export function getCandidateViewState(candidate: DiscoveryCandidate): CandidateViewState {
  ...
  if (candidate.duplicateCheckStatus === "available") {
    if (candidate.verificationStatus === "insufficient") return "duplicate_passed";

    const sourceState = getSearchStageViewState(candidate);
    if (sourceState === "unmapped") return "unmapped";

    // 중복 확인은 FixUp 판정만 책임진다. available이면 Instagram/followers 확인 전이라도 바로 중복 통과로 보낸다.
    // 단, 과거에 Instagram에서 직접 확인한 정확한 followers가 이미 있고 현재 정책 범위를 벗어나면 다시 노출하지 않는다.
    if (
      candidate.followers !== null
      && candidate.followersSource === "instagram"
      && getFollowerPolicyRejection(candidate.category, candidate.followers)
    ) {
      return "unmapped";
    }
